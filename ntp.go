...
type AuthOptions struct {
	...
	Key   string   // symmetric key value
	KeyID uint16   // symmetric key identifier
}
...
func getTime(host string, opt QueryOptions) (*msg, ntpTime, error) {
	...
	var authErr error
	if opt.Auth.Type != AuthNone {
		authErr = verifyMAC(recvBuf, opt.Auth)
	}
...
func verifyMAC(buf []byte, opt AuthOptions) error {
	// Determine the expected size of the MAC.
	macLength := 24
	if opt.Type == AuthMD5 {
		macLength = 20
	}
	...
	// contain a MAC. The size of the message must also be a multiple of 4
	// octets.
	remain := len(buf) - 48
	if remain < macLength || (remain%4) != 0 {
		return ErrAuthFailed
	}

	// The key ID returned by the server must be the same as the key ID sent
	// to the server.
	mac := buf[len(buf)-macLength:]
	keyID := binary.BigEndian.Uint32(mac[:4])
	if keyID != uint32(opt.KeyID) {
		return ErrAuthFailed
	}

	// Calculate the digest of key+header.
	header := buf[:48]

...
func TestOfflineV4ConvertLong(t *testing.T) {
	ts := []timestampV4{0x0, 0xff800000, 0x1ff800000, 0x80000000ff800000, 0xffffffffff800000}
	for _, v := range ts {
		assert.Equal(t, v, toTimestampV4(v.Time()))
	}
}
...
func TestOfflineV4MinError(t *testing.T) {
	...
	h := &headerV4{
		Stratum:       1,
		ReferenceID:   refID,
		ReferenceTime: toTimestampV4(start),
		OriginTime:    toTimestampV4(start.Add(1 * time.Second)),
		ReceiveTime:   toTimestampV4(start.Add(2 * time.Second)),
		TransmitTime:  toTimestampV4(start.Add(3 * time.Second)),
	}
	r := generateResponse(h, toTimestampV4(start.Add(4*time.Second)), nil)
	assertValid(t, r)
		...
		for rec := 1 * time.Second; rec <= 10*time.Second; rec += time.Second {
			for xmt := rec; xmt <= 10*time.Second; xmt += time.Second {
				for dst := org; dst <= 10*time.Second; dst += time.Second {
					h.OriginTime = toTimestampV4(start.Add(org))
					h.ReceiveTime = toTimestampV4(start.Add(rec))
					h.TransmitTime = toTimestampV4(start.Add(xmt))
					r = generateResponse(h, toTimestampV4(start.Add(dst)), nil)
					assertValid(t, r)
...
func TestOfflineV4OffsetCalculation(t *testing.T) {
	now := time.Now()
	t1 := toTimestampV4(now)
	t2 := toTimestampV4(now.Add(20 * time.Second))
	t3 := toTimestampV4(now.Add(21 * time.Second))
	t4 := toTimestampV4(now.Add(5 * time.Second))
...
func TestOfflineV4OffsetCalculationNegative(t *testing.T) {
	now := time.Now()
	t1 := toTimestampV4(now.Add(101 * time.Second))
	t2 := toTimestampV4(now.Add(102 * time.Second))
	t3 := toTimestampV4(now.Add(103 * time.Second))
	t4 := toTimestampV4(now.Add(105 * time.Second))
...
func TestOfflineV4OffsetRollover(t *testing.T) {
	...
	for _, c := range cases {
		...
		org := toTimestampV4(clientTime)
		rec := toTimestampV4(serverTime)
		xmt := toTimestampV4(serverTime.Add(1 * time.Second))
		dst := toTimestampV4(clientTime.Add(1 * time.Second))
...
func TestOfflineV4TimeRollover(t *testing.T) {
	...
	for _, c := range cases {
		...
		assert.Equal(t, c.timestamp, toTimestampV4(tm))
	}
}
...
func TestOfflineV4TimeConversions(t *testing.T) {
	nowNtp := toTimestampV4(time.Now())
	now := nowNtp.Time()
	startNow := now
	for i := 0; i < 100; i++ {
		nowNtp = toTimestampV4(now)
		now = nowNtp.Time()
	}
